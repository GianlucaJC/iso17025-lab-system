                ...
                <form method="POST" action="{{ $is_edit ? route('test-b.update', $test_b_result->id) : route('test-b.store', $acceptance->id) }}" class="needs-validation" novalidate>
                    ...
                    <fieldset class="mb-4 p-3 border rounded {{ $details['color'] }}">
                        ...
                        <div class="row g-3 mb-3">
                            <div class="col-md-4">
                                <label for="incubator_{{ $temp }}" class="form-label">Incubatore</label>
                                <input type="text" class="form-control" id="incubator_{{ $temp }}" name="incubator_{{ $temp }}" value="{{ old('incubator_'.$temp, $is_edit ? $test_b_result->{'incubator_'.$temp} : '') }}" {{ $is_readonly ? 'disabled' : '' }}>
                            </div>
                            <div class="col-md-4">
                                <label class="form-label">Inizio Incubazione</label>
                                <div class="input-group">
                                    <input type="date" class="form-control" name="incubation_start_date_{{ $temp }}" value="{{ old('incubation_start_date_'.$temp, $is_edit && $test_b_result->{'incubation_start_datetime_'.$temp} ? $test_b_result->{'incubation_start_datetime_'.$temp}->format('Y-m-d') : '') }}" {{ $is_readonly ? 'disabled' : '' }}>
                                    <input type="time" class="form-control" name="incubation_start_time_{{ $temp }}" value="{{ old('incubation_start_time_'.$temp, $is_edit && $test_b_result->{'incubation_start_datetime_'.$temp} ? $test_b_result->{'incubation_start_datetime_'.$temp}->format('H:i') : '') }}" {{ $is_readonly ? 'disabled' : '' }}>
                                </div>
                            </div>
                            <div class="col-md-4">
                                <label class="form-label">Fine Incubazione</label>
                                <div class="input-group">
                                    <input type="date" class="form-control" name="incubation_end_date_{{ $temp }}" value="{{ old('incubation_end_date_'.$temp, $is_edit && $test_b_result->{'incubation_end_datetime_'.$temp} ? $test_b_result->{'incubation_end_datetime_'.$temp}->format('Y-m-d') : '') }}" {{ $is_readonly ? 'disabled' : '' }}>
                                    <input type="time" class="form-control" name="incubation_end_time_{{ $temp }}" value="{{ old('incubation_end_time_'.$temp, $is_edit && $test_b_result->{'incubation_end_datetime_'.$temp} ? $test_b_result->{'incubation_end_datetime_'.$temp}->format('H:i') : '') }}" {{ $is_readonly ? 'disabled' : '' }}>
                                </div>
                            </div>
                            <div class="col-md-4">
                                <label for="temperature_{{ $temp }}" class="form-label">Temperatura (°C)</label>
                                <input type="number" step="0.1" class="form-control" id="temperature_{{ $temp }}" name="temperature_{{ $temp }}" value="{{ old('temperature_'.$temp, $is_edit ? $test_b_result->{'temperature_'.$temp} : '') }}" {{ $is_readonly ? 'disabled' : '' }}>
                            </div>
                        </div>
                        ...
                        <table class="table table-bordered text-center">
                            ...
                            <tbody>
                                ...
                                <tr>
                                    <td>{{ $label }}</td>
                                    <td><span class="badge bg-secondary">{{ $test_b_plates[$key] }}</span></td>
                                    @php
                                        $fieldName = 'growth_result_'.$temp.'_'.$key;
